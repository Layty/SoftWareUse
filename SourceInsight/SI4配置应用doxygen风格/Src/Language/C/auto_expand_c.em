...
//能够自动在函数的出、入口首尾加入打印代码，即在光标处加入一个进入函数的打印，在函数的返回处加入一个出函数的打印，用于调试跟踪时很方便
...
//删除InsertTraceInfo()添加的打印信息，定义为菜单
...
//删除文件中全部的InsertTraceInfo()添加的打印信息，定义为菜单
...
//删除工程中全部的InsertTraceInfo()添加的打印信息，定义为菜单
...
macro InsertFileHeaderEN(hbuf, ln,szName,szContent)
{
	...
	InsBufLine_(hbuf,ln + 1)	
    InsBufLine(hbuf, ln + 2,  " * @@file    : @szfilename@")
    InsBufLine(hbuf, ln + 3,  " * @@version : Initial Draft")
    InsBufLine(hbuf, ln + 4,  " * @@date	: @sz@/@sz1@/@sz3@")
    nlnDesc = ln
    iLen = strlen (szContent)
    InsBufLine(hbuf, ln + 5, " * @@brief	: @szContent@")
    InsBufLine(hbuf, ln + 6, " * @@List 	:")
  
    //插入函数列表
    ln = InsertFileList(hbuf,hnewbuf,ln + 7) - 7
    closebuf(hnewbuf)

	InsBufLine_(hbuf,ln+7)
	InsBufLine(hbuf, ln + 8, " */")
	
	InsBufLine(hbuf, ln + 9,  "")
    InsBufLine(hbuf, ln + 10, "/* Private define -----------------------------------------------------------------*/")
    InsBufLine(hbuf, ln + 11, "/* Private macro ------------------------------------------------------------------*/")
    InsBufLine(hbuf, ln + 12, "/* Private function prototypes ----------------------------------------------------*/")
    InsBufLine(hbuf, ln + 13, "/* Private variables --------------------------------------------------------------*/")
    InsBufLine(hbuf, ln + 14, "/* Private functions --------------------------------------------------------------*/")
    ...
    DelBufLine(hbuf,nlnDesc +5)
    
    //注释输出处理,自动换行
    CommentContent(hbuf,nlnDesc + 5," * @@brief	: ",szContent,0)
...
macro InsertFileHeaderCN(hbuf, ln,szName,szContent)
{
    ...
    InsBufLine(hbuf, ln + 2,  "                  版权所有 (C), 2001-2011, AUX_sanxing")
...
//能够自动将一行长的文字分成多行，并且从第二行开始，起始列为关标所在列
macro FormatLine()
{
    hwnd = GetCurrentWnd()
    if (hwnd == 0)
...
macro FuncHeadCommentEN(hbuf, ln, szFunc, szMyName,newFunc)
{
    ...
    InsBufLine(hbuf, ln+3, " * @@return		None")
    InsBufLine(hbuf, ln+4, " * @@retval:		@szRet@")
    InsBufLine_(hbuf,ln+5)
    ln=ln+1
...
//扩展大括号,如果有选中内容把他们框在里面,还带有自动扩展伸缩等级的功能
