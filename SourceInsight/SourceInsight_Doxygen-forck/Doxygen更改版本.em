...
/*            InsBufLine(hbuf, ln + 4, "@szLine1@" # "else");
    ...
    {   
        ...
        InsBufLine(hbuf, ln, "@szLine1@/* Promblem Number: @szQuestion@     Author:@szMyName@,  
 Date:@sz@/@sz1@/@sz3@ ");
    ...
    else if (szCmd == "ab")
    {
        ...
        {
            InsBufLine(hbuf, ln, "@szLine1@/* BEGIN: Added by @szMyName@, @sz@/@sz1@/@sz3@   PN:
@szQuestion@ */");
        }
        else
        {
    ...
    else if (szCmd == "db")
    {
        ...
        szQuestion = GetReg ("PNO")
            if(strlen(szQuestion) > 0)
        {
            InsBufLine(hbuf, ln, "@szLine1@/* BEGIN: Deleted by @szMyName@, @sz@/@sz1@/@sz3@   P
N:@szQuestion@ */");
        }
        else
        {
...
/*            InsBufLine(hbuf, ln + 4, "@szLine1@" # "else");
    ...
    {   
        ...
        DelBufLine(hbuf, ln)
        szQuestion = AddPromblemNo()
        InsBufLine(hbuf, ln, "@szLine1@/* 问 题 单: @szQuestion@     修改人:@szMyName@,   时间:@sz@/@sz1
@/@sz3@ ");
        szContent = Ask("修改原因")
    ...
    else if (szCmd == "ab")
    {
        ...
        if(strlen(szQuestion)>0)
        {
            InsBufLine(hbuf, ln, "@szLine1@/* BEGIN: Added by @szMyName@, @sz@/@sz1@/@sz3@   问题单
号:@szQuestion@ */");
        }
        else
        {
    ...
    else if (szCmd == "db")
    {
        ...
        if(strlen(szQuestion) > 0)
        {
            InsBufLine(hbuf, ln, "@szLine1@/* BEGIN: Deleted by @szMyName@, @sz@/@sz1@/@sz3@   问
题单号:@szQuestion@ */");
        }
        else
        {
...
macro InsertFileHeaderEN(hbuf, ln,szName,szContent)
{
    ...
    InsBufLine(hbuf, ln + 6,  " * @@par				Copyright (C),  2013-2023, Sky Tech")
...
macro SkipCommentFromString(szLine,isCommentEnd)
{
    ...
    while(nIdx < nLen )
    {
        //如果当前行开始还是被注释，或遇到了注释开始的变标记，注释内容改为空格?        if( (isCommentEnd == 0) || (szLine[nIdx] == "/" && szLine[nIdx+1] == "*"))
        {
            fIsEnd = 0
            while(nIdx < nLen )
            {
