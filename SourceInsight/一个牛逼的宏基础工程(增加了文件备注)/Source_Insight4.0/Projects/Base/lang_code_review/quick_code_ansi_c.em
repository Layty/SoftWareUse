...
macro smart_expand_c()
{
    ...
    /*配置命令执行*/
    else if (wordinfo.szWord == "config" || wordinfo.szWord == "co"||wordinfo.szWord == "CO")
    {
        DelBufLine(hbuf, ln)
        ConfigureSystem()
        return
    }
    else if(wordinfo.szWord == "cofig_trace" || wordinfo.szWord == "ct"|| wordinfo.szWord == "CT")//配置trace 语句
    {
        DelBufLine(hbuf, ln)
        ConfigureTrace()
    }
    ...
    else if(wordinfo.szWord == "trace0")
    {

    
    }
    else if(wordinfo.szWord == "trace1")
    {

    
    }
    else if(wordinfo.szWord == "trace2")
    {

    
    }

	szMyName = PromptRegName("Enter your name:","请输入您的名字:","MYNAME_EN","MYNAME_CN");
    if(language == 1)
    {
...
macro InsertTraceInCurFunction(hbuf,symbol)
{
    ...
    nExitCount = 1;
    szTraceName = GetReg("TRACE_NAME")
    if(Nil == szTraceName)
    {
       szTraceName = "DebugTrace"
    }

    InsBufLine(hbuf, ln, "    @szTraceName@(\"\\r\\n |@symbolname@() entry--- \");")
    ln = ln + 1
    fIsEnd = 1
    fIsNeedPrt = 1
    fIsSatementEnd = 1
...
macro AutoInsertTraceInfoInPrj()
{
    ...
    while (ifile < ifileMax)
    {
        ...
        szExt = toupper(public_get_file_ext(filename))
...
macro CreateFunctionDef(hbuf, szName, language)
{
    ...
    {    
        ...
        szExt = public_get_file_ext(szFileName)        
...
macro CreateNewHeaderFile()
{
   ...
   if(language == 1)
    {
		 szMyName = getreg(MYNAME_CN)
		if(strlen( szMyName ) == 0)
		{
			szMyName = Ask("请输入您的名字:")
			setreg(MYNAME_CN, szMyName)
		}
    }
    else
    {
		szMyName = getreg(MYNAME_EN)
		if(strlen( szMyName ) == 0)
		{
			szMyName = Ask("Enter your name:")
			setreg(MYNAME_EN, szMyName)
		}
    }
    isymMax = GetBufSymCount(hbuf)
    isym = 0
...
macro InsertFileHeaderEN(hbuf, ln,szName,szContent)
{
    ...
    szCompany = getreg(MYCOMPANY_EN)
    if(strlen( szCompany ) == 0)
    {
        szCompany = Ask("Enter your Company:")
        setreg(MYCOMPANY_EN, szCompany)
    }
    GetFunctionList(hbuf,hnewbuf)
    InsBufLine(hbuf, ln + 0,  "/******************************************************************************")
    InsBufLine(hbuf, ln + 1,  "")
    InsBufLine(hbuf, ln + 2,  "  Copyright (C), 2001-2016, @szCompany@ All Rights Reserved")
...
/*            InsBufLine(hbuf, ln + 4, "@szLine1@" # "else");
    ...
    else if(szCmd == "trace0")
    {
        
    }
    else if(szCmd == "trace1")
    {
    
    }
    else if(szCmd == "trace2")
    {
    
    }
    else if(szCmd == "trace3")
    {
...
macro InsertFileHeaderCN(hbuf, ln,szName,szContent)
{
    ...
    if(hnewbuf == hNil)
    {
     ...
     szCompany = getreg(MYCOMPANY_CN)
    if(strlen( szCompany ) == 0)
    {
        szCompany = Ask("请输入你的公司名称:")
        szCompany = Ask("请输入你的公司名称:")
        setreg(MYCOMPANY_CN, szCompany)
    }
    GetFunctionList(hbuf,hnewbuf)
    InsBufLine(hbuf, ln + 0,  "/******************************************************************************")
    InsBufLine(hbuf, ln + 1,  "")
    InsBufLine(hbuf, ln + 2,  "                  版权(C), 2001-2016, @szCompany@ 版权所有")
    InsBufLine(hbuf, ln + 3,  "")
    InsBufLine(hbuf, ln + 4,  " ******************************************************************************")
    sz = GetFileName(GetBufName (hbuf))
